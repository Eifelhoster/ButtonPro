...
class EBP_Editor {
	...
	public function __construct() {
		...
		// Load frontend stylesheet inside the TinyMCE editor iframe so buttons
		// look identical to their frontend appearance.
		add_filter( 'mce_css', array( $this, 'mce_css' ) );

		// AJAX handler: server-renders a [eifelhoster_button] shortcode so the
		// TinyMCE view can display the button visually in the editor.
		add_action( 'wp_ajax_ebp_render_button', array( $this, 'ajax_render_button' ) );
	}
	...
	public function enqueue( $hook ) {
		...
		// Register the TinyMCE view script: loads in the page (not inside TinyMCE)
		// so wp.mce.views.register() is called before TinyMCE processes content.
		wp_enqueue_script(
			'ebp-mce-view',
			EBP_PLUGIN_URL . 'assets/js/ebp-mce-view.js',
			array( 'jquery', 'mce-view' ),
			EBP_VERSION,
			false // load in <head> so the view is registered before TinyMCE initialises
		);
		wp_localize_script( 'ebp-mce-view', 'ebpMceData', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'nonce'   => wp_create_nonce( 'ebp_render_nonce' ),
		) );
	}

	/**
	 * Append the plugin's frontend stylesheet to the TinyMCE editor iframe so
	 * rendered buttons look the same as on the frontend.
	 *
	 * @param string $mce_css Comma-separated list of stylesheet URLs.
	 * @return string
	 */
	public function mce_css( $mce_css ) {
		$url = EBP_PLUGIN_URL . 'assets/css/ebp-frontend.css';
		return $mce_css ? $mce_css . ',' . $url : $url;
	}

	/**
	 * AJAX handler: render a [eifelhoster_button] shortcode server-side and
	 * return the HTML so the TinyMCE view can display it visually.
	 *
	 * Requires a valid nonce and edit_posts capability.
	 */
	public function ajax_render_button() {
		check_ajax_referer( 'ebp_render_nonce', 'nonce' );
		...
		// Only allow our own shortcode tag – reject anything else.
		if ( ! preg_match( '/^\[eifelhoster_button(\s[^\]]*)?]$/', trim( $shortcode ) ) ) {
			wp_send_json_error( array( 'message' => 'Invalid shortcode' ) );
		}
